...
const hiddenExamples = new Set([
  "13_data_connectors/01_sqlite_connector.py",
  "13_data_connectors/02_postgres_connector.py",
]);

const databasesByExample = {
  "13_data_connectors/03_postgres_agent.py": ["PostgreSQL"],
  "13_data_connectors/04_sql_snapshot_knowledge_agent.py": ["PostgreSQL"],
...
const files = (await pythonFiles(examplesRoot)).filter((file) => !hiddenExamples.has(path.relative(examplesRoot, file))).sort();
const examples = await Promise.all(files.map(async (file) => {
  ...
  return {
    ...
    databases: databasesByExample[relativePath],
